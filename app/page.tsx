...
const skillGroups = [
  { icon: Code2, label: 'Languages & Backend', items: ['Python', 'SQL / MySQL', 'JavaScript', 'FastAPI', 'REST APIs', 'Scripting & Automation'] },
  { icon: BarChart3, label: 'Data Analysis & Visualization', items: ['Pandas', 'NumPy', 'EDA', 'Power BI', 'Matplotlib', 'Seaborn', 'Pinecone', 'Advanced Google Sheets'] },
  { icon: BrainCircuit, label: 'Machine Learning & Deep Learning', items: ['Scikit-learn', 'TensorFlow', 'PyTorch (familiar)', 'Supervised / Unsupervised Learning', 'Model Evaluation', 'Statistics & Probability'] },
  { icon: Network, label: 'NLP & Generative AI', items: ['RAG Pipelines', 'LLM Integration', 'Embeddings & Semantic Search', 'Agentic Workflows', 'Claude · ChatGPT · Codex', 'Groq API'] },
  { icon: Layers3, label: 'Frontend & Web', items: ['React.js', 'Next.js (familiar)', 'HTML5', 'CSS3', 'Vercel (familiar)'] },
  { icon: Database, label: 'Cloud & Dev Tools', items: ['Azure (familiar)', 'Git', 'GitHub', 'Jupyter Notebook', 'VS Code', 'Cloud Computing Fundamentals'] },
]
...
const experiences = [
  ...
  { company: 'Exafluence Pvt Ltd', role: 'Python Full Stack Web Development Trainee', duration: 'Dec 2022 — Jun 2023', location: 'Tirupati, Andhra Pradesh', detail: 'Worked on Python backend development and SQL database operations, building and testing web application modules with backend logic and API workflows.' },
  { company: 'Hyniva Consultancy Services', role: 'React Frontend Developer Intern', duration: '2 months · On-site', location: 'Nagavara, Bangalore', detail: 'Built responsive React.js user interfaces and strengthened practical frontend development skills across web application workflows.' },
]
...
const projects = [
  { number: '01', title: 'SmartDocQA', type: 'RAG · Research', description: 'A centralized multi-tenant Retrieval-Augmented Generation platform for securely managing and querying organizational knowledge bases.', details: ['FastAPI PDF ingestion and intelligent chunking', 'Pinecone embeddings with company-specific indexes', 'Groq-powered contextual answers with organization-level access control', 'React.js authentication and document management frontend'], technologies: ['Python', 'FastAPI', 'React.js', 'Pinecone', 'Groq'], featured: true },
]
...
export default function Page() {
  ...
  return (
    <main>
      ...
      <section className="section shell reveal" id="skills"><div className="section-heading"><div><p className="eyebrow">01 / Capabilities</p><h2>Tools for turning<br /><em>questions into clarity.</em></h2></div><p className="section-intro">From preparing messy data to building intelligent interfaces, I enjoy working across the full path from a problem to a working solution.</p></div><div className="skill-grid">{skillGroups.map(({ icon: Icon, label, items }) => <article className="skill-card" key={label}><div className="skill-top"><Icon size={20} /><span>0{skillGroups.findIndex((g) => g.label === label) + 1}</span></div><h3>{label}</h3><div className="tag-list">{items.map((item) => <span key={item}>{item}</span>)}</div></article>)}</div></section>

      <section className="section section-muted reveal" id="experience"><div className="shell"><div className="section-heading compact"><div><p className="eyebrow">02 / Experience</p><h2>Learning by<br /><em>doing the work.</em></h2></div><p className="section-intro">Practical experience across data science, analytics, Python backend development, React frontend engineering, and production-minded AI/ML workflows.</p></div><div className="timeline">{experiences.map((experience, index) => <article className="timeline-item" key={`${experience.company}-${experience.role}`}><div className="timeline-marker">0{index + 1}</div><div className="timeline-content"><div className="timeline-head"><div><p className="company">{experience.company}</p><h3>{experience.role}</h3></div><span>{experience.duration}</span></div><p className="timeline-location">{experience.location}</p><p>{experience.detail}</p></div></article>)}</div></div></section>
...
<section className="section shell reveal" id="education"><div className="section-heading compact"><div><p className="eyebrow">03 / Education</p><h2>Built on a<br /><em>strong foundation.</em></h2></div><p className="section-intro">Academic training in artificial intelligence, machine learning, computer science, and data-driven problem solving.</p></div><div className="education-grid">{education.map((item, index) => <article className="education-card" key={item.degree}><div className="education-index">0{index + 1}</div><div><h3>{item.degree}</h3><p>{item.school}</p><span>{item.place} · {item.duration}</span></div><strong>{item.result}</strong></article>)}</div></section>
      ...
      <section className="section shell reveal" id="projects"><div className="section-heading"><div><p className="eyebrow">04 / Selected work</p><h2>Ideas made<br /><em>tangible.</em></h2></div><p className="section-intro">A selection of projects exploring retrieval, prediction, and the practical side of machine learning.</p></div><div className="project-grid">{projects.map((project) => <article className={`project-card ${project.featured ? 'featured' : ''}`} key={project.title}><div className="project-head"><span className="project-number">{project.number}</span><span className="project-type">{project.type}</span><a className="project-arrow-link" href="https://github.com/Teja-1910" target="_blank" rel="noreferrer" aria-label="Open Brahma Teja Reddy GitHub profile"><ArrowUpRight className="project-arrow" size={20} /></a></div><div className="project-body"><div><h3>{project.title}</h3><p>{project.description}</p><ul>{project.details.map((detail) => <li key={detail}><Check size={14} />{detail}</li>)}</ul><div className="project-tags">{project.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div><a className="repo-link" href="https://github.com/Teja-1910/_SmartDocQA_.git" target="_blank" rel="noreferrer">Open SmartDocQA repository <ArrowUpRight size={15} /></a></div></div></article>)}</div></section>
